...
import { ViewHomeImageDialogTrigger } from "../components"
import { ViewHomeDataProvider } from "../context"
import { useViewHomeData } from "../hooks"
...
export const ViewHomeLayout = ({ children }) => {
  ...
  const [isViewHomeImageDialogOpen, setIsViewHomeImageDialogOpen] =
    useState(false)
  ...
  useEffect(() => {
    const getImagesBlobUrl = async () => {
      try {
        ...
        const imagesBlobUrlData = Promise.all(
          homeAttributes?.photos?.map(async (photo) => {
    ...
    if (homeAttributes?.photos?.length >= 1) {
      getImagesBlobUrl()
    }
  }, [homeAttributes])
  ...
  const handlePDF = async () => {
    ...
    const payloadItem = {
      property_type: "Home",
      kode_propar: homeAttributes.kode_propar,
      facilities_id:
        homeAttributes.facilities.map((facility) => facility.id) || [],
      photos_id: homeAttributes.photos.map((photo) => photo.id) || [],
      area_id: homeAttributes.property_area.id,
      payment_term_id: homeAttributes.property_payment_term.id,
  ...
  return (
    <>
      <Head>
        <title>
          {query.slug} - Home Property View | Noble Property Asia Database
          Management System
        </title>
      </Head>
      <ToastProvider>
        ...
        <div className="flex flex-col gap-5 p-4 bg-white rounded-md md:p-8 xl:px-12">
          {(homeDataIsLoading || Object.keys(homeAttributes).length < 1) && (
            ...
            <>
              ...
              <section className="flex items-center justify-center h-max">
                {imagesAreLoading ? (
                  ...
                  <DialogRadix.Root open={isViewHomeImageDialogOpen}>
                    <div className="grid w-full grid-rows-[max-content_max-content] gap-4 2xl:grid-cols-[4fr_1fr] 2xl:grid-rows-1">
                      {imagesBlobUrl.length > 0 && (
                        <ViewHomeImageDialogTrigger
                          onClick={() => {
                            setIsViewHomeImageDialogOpen(true)
                    ...
                    <DialogRadix.Portal>
                      <DialogRadix.Overlay
                        ...
                        onClick={() => setIsViewHomeImageDialogOpen(false)}
                      />
                ...
                <section className="flex flex-col items-center gap-2 p-6 rounded-md shadow-sm h-max border-1 border-npa-neutral-200 lg:order-last">
                  <div className="flex flex-col items-center gap-1 text-center">
                    <h2 className="text-2xl font-semibold text-npa-info-400">
                      {homeAttributes.selling_price !== 0
                  ...
                  <section className="flex flex-col items-center w-2/3 gap-4 lg:items-start">
                    ...
                    <div className="flex flex-col gap-3 text-center lg:text-start">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <p className="font-medium">
                          {homeAttributes.convertedPrices.sellingPrice}
                        </p>
                      </div>
                      <div className="flex flex-col gap-1 text-center lg:text-start">
                        <h4 className="font-medium text-npa-neutral-500">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <p className="font-medium">
                          {homeAttributes?.compound_fee_coverage || "-"}
                        </p>
                      </div>
                    </div>
                  </section>
                ...
                <section className="flex flex-col gap-6 overflow-x-hidden">
                  <div className="flex items-center gap-4 py-4 overflow-x-auto scrollbar-thin scrollbar-track-npa-neutral-200 scrollbar-thumb-npa-neutral-400/60 scrollbar-thumb-rounded-lg lg:gap-8 xl:gap-12">
                    {tabsDetails.map(({ value, title, href }) => (
                      <Link
                        href={`/home-management/view/${query.slug}/${href}`}
...
export const getViewHomeLayoutProps = (page) =>
  getAdminLayout(
    <ViewHomeDataProvider>
      <ViewHomeLayout>{page}</ViewHomeLayout>
    </ViewHomeDataProvider>
  )
