...
const AddHomeLayout = ({ children }) => {
  ...
  const onSubmit = async ({ images, ...payload }) => {
    ...
    const { data } = await instance.post(
      process.env.NEXT_PUBLIC_ENDPOINT_HOME_CREATE,
      formData,
      {
        headers: {
  ...
  return (
    <ToastProvider swipeDirection="right" duration={5000}>
      <CreateNewItemLayout
        ...
        sectionItems={ADD_HOME_SECTION_ITEMS}
      >
        {children}
      </CreateNewItemLayout>
      {toastState.open && (
...
export const getAddHomeLayout = (page) => {
  return getAdminLayout(<AddHomeLayout>{page}</AddHomeLayout>)
}

export default AddHomeLayout
