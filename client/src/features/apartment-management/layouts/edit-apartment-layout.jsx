...
const EDIT_APARTMENT_SECTION_ITEMS = [
  {
    numberOfSection: 1,
...
export const EditApartmentLayout = ({ children }) => {
  ...
  const methods = useForm({
    ...
    defaultValues: {
      ...
      study_room: "",
      available: "",
      pic: { fullname: "", role: "", company: "", phone_number: "" },
      fees: {
        rental_price: "",
        selling_price: "",
        price_currency: "",
  ...
  const onSubmit = async ({ images, ...payload }) => {
    ...
    const endpoint = `${process.env.NEXT_PUBLIC_ENDPOINT_APARTMENT_UPDATE}/${query.slug}`
    ...
    if (status === 204) {
      ...
      handleToggleToast({
        open: true,
        variant: "success",
        message: "Apartment has been updated successfully",
      })
    }
  }
  ...
  return (
    <ToastProvider swipeDirection="right" duration={5000}>
      <EditItemLayout
        ...
        sectionItems={EDIT_APARTMENT_SECTION_ITEMS}
      >
        {children}
      </EditItemLayout>
...
export const getEditApartmentLayout = (page) => {
  return getAdminLayout(<EditApartmentLayout>{page}</EditApartmentLayout>)
}
