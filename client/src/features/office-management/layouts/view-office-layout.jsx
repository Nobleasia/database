...
export const ViewOfficeLayout = ({ children }) => {
  ...
  const [isViewOfficeImageDialogOpen, setIsViewOfficeImageDialogOpen] =
    useState(false)
  ...
  useEffect(() => {
    const getImagesBlobUrl = async () => {
      try {
        ...
        const imagesBlobUrlData = Promise.all(
          officeAttributes?.photos?.map(async (photo) => {
    ...
    if (officeAttributes?.photos?.length >= 1) {
      getImagesBlobUrl()
    }
  }, [officeAttributes])
  ...
  return (
    <>
      <Head>
        <title>
          {query.slug} - Office Property View | Noble Property Asia Database
          Management System
        </title>
      </Head>
      <HeaderPage>
...
export const getViewOfficeLayoutProps = (page) =>
  getAdminLayout(
    <ViewOfficeDataProvider>
      <ViewOfficeLayout>{page}</ViewOfficeLayout>
    </ViewOfficeDataProvider>
  )
