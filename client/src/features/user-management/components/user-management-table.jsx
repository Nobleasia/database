...
const UserManagementTableCell = ({ children }) => {
  return (
...
export const UserManagementTable = ({ isLoading, table }) => {
  return (
    <div className="relative">
      ...
      <div
        className={cn(
          "relative flex max-h-96 w-full flex-col rounded-xl backdrop-blur-md",
          {
            ...
            {table.getRowModel().rows.map((row, index) => (
              ...
              >
                {row.getVisibleCells().map((cell) => {
                  return (
                    <UserManagementTableCell key={cell.id}>
                      {flexRender(
                        cell.column.columnDef.cell,
                        cell.getContext()
                      )}
                    </UserManagementTableCell>
                  );
                })}
              </TableRow>
            ))}
