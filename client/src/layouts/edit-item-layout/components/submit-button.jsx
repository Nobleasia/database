...
export const SubmitButton = ({
  ...
  const isButtonDisabled = useMemo(() => {
    return isSubmitting || showAndCloseOnly || !isDirtyFields;
  }, [isSubmitting, showAndCloseOnly, isDirtyFields]);
      ...
      <AlertDialogContent className="fixed top-1/2 left-1/2 z-50 flex h-max w-full max-w-md -translate-x-1/2 -translate-y-1/2 flex-col gap-8 rounded-xl bg-white p-6">
        <div className="flex flex-col gap-3">
          ...
          <AlertDialogDescription className="opacity-70">
            This action will update {sectionRoot.toLowerCase()} data
        ...
        <div className="flex w-full items-center justify-end gap-5">
          ...
          <AlertDialogAction asChild>
            <Button
              ...
              className="flex items-center gap-2 rounded-md bg-npa-info-400/80 px-4 py-3 font-medium text-npa-neutral-25 duration-200 disabled:cursor-not-allowed disabled:opacity-80 hover:bg-npa-info-400 hover:disabled:bg-npa-info-400/80"
            >
              Confirm
            </Button>
          </AlertDialogAction>
