...
const updateExistingUserByUsername = async (req) => {
  ...
  if (username) {
    ...
    await Log.create({
      ...
      message: `Update User ${user.username} Username to ${username}`,
    });
  }
  ...
  if (fullname) {
    ...
    await Log.create({
      status_code: 204,
      username: req.username ?? null,
      message: `Update User ${user.username} Fullname`,
    });
  }
  ...
  if (role) {
    ...
    if (!(role === "superadmin" || role === "admin" || role === "user")) {
      const err = new ErrorDetails(
        "UserError",
        "role",
        "role value must be between `superadmin`, `admin`, or `user`",
      );
      // TODO: ganti console ke log kalau sudah mau production
      console.error(err);
      throw new ErrorResponse(400, "BAD_REQUEST", {
    ...
    await Log.create({
      status_code: 204,
      username: req.username ?? null,
      message: `Update User ${user.username} Role`,
    });
  }
  ...
  if (password) {
    ...
    await Log.create({
      status_code: 204,
      username: req.username ?? null,
      message: `Update User ${user.username} Password`,
    });
  }
