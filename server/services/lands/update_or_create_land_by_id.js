...
const updateOrCreateLandById = async (req) => {
  ...
  const { fees, pic, deleted_photo_ids, restored_photo_ids, ...newLand } = req.body;
    ...
    await sequelize.transaction(async (t) => {
      ...
      if (landPhotos) {
        if (!(Array.isArray(landPhotos))) {
          const err = new ErrorDetails("LandPhotoError", "deleted_photo_ids", "deleted_photo_ids must be array");
          // TODO: ganti console ke log kalau sudah mau production
          console.error(err);
          throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
        }
        ...
        if (landPhotos.length < 1) {
          const err = new ErrorDetails("LandPhotoError", "deleted_photo_ids", "deleted_photo_ids must have a minimum of 1 item");
          // TODO: ganti console ke log kalau sudah mau production
          console.error(err);
          throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
        }
        ...
        for (const photo of landPhotos) {
          ...
          if (!photo.id) {
            const err = new ErrorDetails("LandPhotoError", "id", "deleted_photo_ids id must not be null");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          if (typeof photo.id !== "number") {
            const err = new ErrorDetails("LandPhotoError", "id", "deleted_photo_ids id must be integer");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          if (photo.force_delete === undefined) {
            const err = new ErrorDetails("LandPhotoError", "force_delete", "force_delete must not be null");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          if (typeof photo.force_delete !== "boolean") {
            const err = new ErrorDetails("LandPhotoError", "force_delete", "force_delete must be boolean");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          if (!existingPhoto) {
            const err = new ErrorDetails("LandPhotoError", "id", "photo id not found");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(404, "NOT_FOUND", { [err.attribute]: err.message });
          }
      ...
      if (restoredPhotos) {
        if (!(Array.isArray(restoredPhotos))) {
          const err = new ErrorDetails("LandPhotoError", "restored_photo_ids", "restored_photo_ids must be array");
          // TODO: ganti console ke log kalau sudah mau production
          console.error(err);
          throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
        }
        ...
        if (restoredPhotos.length < 1) {
          const err = new ErrorDetails("LandPhotoError", "restored_photo_ids", "restored_photo_ids must have a minimum of 1 item");
          // TODO: ganti console ke log kalau sudah mau production
          console.error(err);
          throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
        }
        ...
        for (const photo of restoredPhotos) {
          if (!photo.id) {
            const err = new ErrorDetails("LandPhotoError", "id", "restored_photo_ids id must not be null");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          if (typeof photo.id !== "number") {
            const err = new ErrorDetails("LandPhotoError", "id", "restored_photo_ids id must be integer");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
      ...
      if (landPic !== null && typeof landPic === "object" && Object.keys(landPic).length !== 0) {
        const where = {};
        ...
        await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} PIC` }, {
          transaction: t,
        });
      }

      if (newLandFees !== null && typeof newLandFees === "object" && Object.keys(newLandFees).length !== 0) {

        if (newLandFees.price_currency) {
          const priceCurrency = newLandFees.price_currency;
          if (typeof priceCurrency !== "string") {
            const err = new ErrorDetails("LandError", "price_currency", "price_currency must be string");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          if (!(priceCurrency === "Rupiah" || priceCurrency === "US Dollar")) {
            const err = new ErrorDetails("LandError", "price_currency", "price_currency must be Rupiah or US Dollar");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Price Currency` }, {
            transaction: t,
          });
        }
        ...
        if (newLandFees.price !== undefined) {
          ...
          if (typeof rentalPrice !== "number") {
            const err = new ErrorDetails("LandError", "price", "price must be integer");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Rental Price` }, {
            transaction: t,
          });
        }
        ...
        if (newLandFees.property_payment_terms_name === "") {
          ...
          if (typeof paymentTermName !== "string") {
            const err = new ErrorDetails("LandError", "property_payment_terms_name", "property_payment_terms_name must be a string");
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Payment Term` }, {
            transaction: t,
          });
        }
        ...
        if (newLandFees.vat_percentage !== undefined) {
          ...
          if (typeof vatPercentage !== "number") {
            const err = new ErrorDetails("LandError", "vat_percentage", "vat_percentage must be integer");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} VAT Percentage` }, {
            transaction: t,
          });
        }
        ...
        if (newLandFees.vat_is_included) {
          ...
          if (typeof vatIsIncluded !== "boolean") {
            const err = new ErrorDetails("LandError", "vat_is_included", "vat_is_included must be boolean");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} VAT is included` }, {
            transaction: t,
          });
        }
        ...
        if (newLandFees.wht_percentage !== undefined) {
          ...
          if (typeof whtPercentage !== "number") {
            const err = new ErrorDetails("LandError", "wht_percentage", "wht_percentage must be integer");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} WHT Percentage` }, {
            transaction: t,
          });
        }
        ...
        if (newLandFees.wht_is_included) {
          ...
          if (typeof whtIsIncluded !== "boolean") {
            const err = new ErrorDetails("LandError", "wht_is_included", "wht_is_included must be boolean");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
        ...
        if (newLandFees.lease_term_time !== undefined) {
          ...
          if (typeof leaseTermTime !== "number") {
            const err = new ErrorDetails("LandError", "lease_term_time", "lease_term_time must be integer");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Lease Term Time` }, {
            transaction: t,
          });
        }
        ...
        if (newLandFees.lease_term_type === "") {
          ...
          if (typeof leaseTermType !== "string") {
            const err = new ErrorDetails("LandError", "lease_term_type", "lease_term_type must be a string");
            // TODO: Ganti console.log ke log kalau sudah dalam mode production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          if (!validLeaseTermTypes.includes(leaseTermType)) {
            const err = new ErrorDetails("LandError", "lease_term_type", "lease_term_type must be Month or Year");
            // TODO: Ganti console.log ke log kalau sudah dalam mode production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Lease Term Type` }, {
            transaction: t,
          });
        }
      }

      if (newLand !== null && typeof newLand === "object" && Object.keys(newLand).length !== 0) {
        if (newLand.address) {
          land.address = newLand.address;

          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Address` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.property_area) {
          ...
          if (area[1]) {
            await Log.create({ status_code: 201, username: req.username ?? null, message: `Property Area ${newLand.property_area} is CREATED` }, {
              transaction: t,
            });
          } else {
            await Log.create({ status_code: 200, username: req.username ?? null, message: `Viewing Property Area ${newLand.property_area} data` }, {
              transaction: t,
            });
          }

          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Area` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.land_size !== undefined) {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Land Size` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.ownership) {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Ownership` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.zone) {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Zone` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.available) {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Available` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.surroundings) {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Surroundings` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.remarks_1 || newLand.remarks_1 == "") {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Remarks 1` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.remarks_2 || newLand.remarks_2 == "") {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Remarks 2` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.remarks_3 || newLand.remarks_3 == "") {
          ...
          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Remarks 3` }, {
            transaction: t,
          });
        }
        ...
        if (newLand.kode_propar) {
          ...
          const existingLand = await Land.findOne({ where: { kode_propar: newLand.kode_propar } });

          if (existingLand && existingLand.id !== land.id) {
            const err = new ErrorDetails("LandError", "kode_propar", "kode_propar has been taken");
            // TODO: ganti console ke log kalau sudah mau production
            console.error(err);
            throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
          }

          await Land.update({ kode_propar: newLand.kode_propar }, {
            where: { kode_propar: land.kode_propar },
          });

          await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} Kode Propar to ${newLand.kode_propar}` }, {
            transaction: t,
          });
        }
      }
      ...
      await Log.create({ status_code: 204, username: req.username ?? null, message: `Update Land ${land.kode_propar} data` }, {
        transaction: t,
      });
    });
  } catch (error) {
