...
const createNewPropertyPartialan = async (req) => {
  const { property_type, kode_propar, photos_id, facilities_id, area_id, payment_term_id, selected_property_data } = req.body;
  ...
  if (photos_id) {
    ...
    if (photos_id.length < 4) {
      const err = new ErrorDetails("PropertyPartialanError", "photos_id", "photos_id must at least have 4 items");
      // TODO: ganti console ke log kalau sudah mau production
      console.error(err);
      throw new ErrorResponse(400, "BAD_REQUEST", { [err.attribute]: err.message });
    }
  ...
  if (property_type === "Apartment") {
    ...
    content = await Apartment.findByPk(kode_propar, {
      attributes: selected_property_data,
      include,
    });

    if (!content) {
      const err = new ErrorDetails("PropertyPartialanError", "content", "property_data not found");
      // TODO: ganti console ke log kalau sudah mau production
      console.error(err);
  ...
  if (property_type === "Home") {
    ...
    content = await Home.findByPk(kode_propar, {
      attributes: selected_property_data,
      include,
    });
  ...
  if (property_type === "Land") {
    ...
    content = await Land.findByPk(kode_propar, {
      attributes: selected_property_data,
      include,
    });
  ...
  if (property_type === "Office") {
    ...
    content = await Office.findByPk(kode_propar, {
      attributes: selected_property_data,
      include,
    });
  ...
  const { id } = await PropertyPartialan.create({ property_type, content });

  await Log.create({ status_code: 201, username: req.username ?? null, message: `Create New Property Partialan - {${id}}` });

  return id;
}

module.exports = createNewPropertyPartialan;
